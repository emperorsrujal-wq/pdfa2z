...
export const PdfSuite: React.FC<PdfSuiteProps> = ({ initialTab = 'CHAT', initialToolMode = 'MENU' }) => {
  ...
  return (
    <div className="max-w-6xl mx-auto min-h-[calc(100vh-14rem)] flex flex-col animate-fade-in px-4">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
          ...
          <div className="flex bg-slate-50 p-1 rounded-2xl border border-slate-100 shadow-inner w-fit">
            <button
              ...
              className={`flex items-center gap-2 px-8 py-3 rounded-xl text-sm font-black uppercase tracking-widest transition-all ${activeTab === 'CHAT'
            ...
            <button
              ...
              className={`flex items-center gap-2 px-8 py-3 rounded-xl text-sm font-black uppercase tracking-widest transition-all ${activeTab === 'TOOLS'
                ? 'bg-indigo-600 text-white shadow-xl'
                : 'text-slate-400 hover:text-slate-900'
                }`}
            >
      ...
      <div className={`flex-1 min-h-0 bg-white ${initialToolMode === 'EDIT' ? '' : 'rounded-[2.5rem] border border-slate-100 shadow-sm'} overflow-hidden p-1`}>
        {activeTab === 'CHAT' ? (
          <PdfAnalyzer />
        ) : (
