 ...
 */
export async function sendLeadNotification(ownerEmail: string, lead: JourneyLead) {
  const subject = `🚀 New Lead Captured: ${lead.journeyTitle}`;
  
  // Extract summary data for the email
  ...
  const html = `
    <div style="font-family: 'Inter', system-ui, sans-serif; max-width: 600px; margin: 0 auto; background: #0f172a; color: #e2e8f0; border-radius: 20px; overflow: hidden; border: 1px solid #1e293b;">
      ...
      <div style="padding: 40px;">
        ...
        <ul style="background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.05); border-radius: 12px; padding: 20px 20px 20px 40px; list-style-type: square; margin: 24px 0;">
          ${mainData}
        </ul>
  ...
  try {
    // Example: fetch('/api/send-email', { method: 'POST', body: JSON.stringify({ to: ownerEmail, subject, html }) })
    console.log('Notification trigger sent to backend services.');
    return true;
  } catch (error) {
