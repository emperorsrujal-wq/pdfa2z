...
export type PdfToolMode = 'MENU' | 'MERGE' | 'SPLIT' | 'TO_IMAGE' | 'COMPRESS' | 'IMG_TO_PDF' | 'IMAGES_TO_PDF' | 'ROTATE' | 'DELETE_PAGES' | 'EXTRACT_TEXT' | 'PAGE_NUMBERS' | 'PROTECT' | 'TO_WORD' | 'WORD_TO_PDF' | 'TO_EXCEL' | 'TO_HTML' | 'UNLOCK' | 'WATERMARK' | 'GRAYSCALE' | 'FLATTEN' | 'REPAIR' | 'METADATA' | 'CHAT' | 'SIGN' | 'ORGANIZE' | 'SANITIZE' | 'REVERSE' | 'EXTRACT_IMAGES' | 'EDIT' | 'CROP' | 'URL_TO_PDF' | 'PDF_TO_CSV' | 'PDF_TO_PPT' | 'PPT_TO_PDF' | 'EPUB_TO_PDF' | 'MOBI_TO_PDF' | 'AZW3_TO_PDF' | 'OUTLOOK_TO_PDF' | 'PDF_TO_TEXT' | 'REDACT' | 'BATCH';
...
export type AiWriterMode = 'GRAMMAR' | 'SUMMARIZE' | 'PARAPHRASE' | 'EMAIL';
